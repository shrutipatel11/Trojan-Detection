...
def schedule_process(processes, tpc_results, cores, edges, start_time, report):
    ...
    while True:
        ...
        for k in range(lp):
            ...
            if k!=lp-1:
                cmdarg+=','
        print "3PC",tname,"scheduled in cpu",cpu
        cmdarg+='A'
...
def receive_results(processes, tpc_results, cores, edges, start_time, report):
    ...
    while True:
        ...
        if ready_sockets:
            ...
            # To check the system in case of wrong output, uncomment the below 2 lines
            # if name=='ifft':
            #     data = data.replace('A',',4444A')
            l = 0
            while l<len(data):
                client[0].send(data[l:l+60000])
                l=l+60000
            cores[j]=0
            time.sleep(0.5)
